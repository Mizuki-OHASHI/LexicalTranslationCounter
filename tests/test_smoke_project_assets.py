...
class SmokeProjectAssetTests(unittest.TestCase):
    def test_de_en_smoke_input_has_required_files(self):
        ...
        for lang in ("de", "en"):
            for pos_name in ("noun", "verb", "adj", "adverb"):
                self.assertTrue(
                    (input_dir / f"wordlist_{lang}_{pos_name}.csv").exists()
                )
    ...
    def test_en_ru_smoke_input_has_required_files(self):
        input_dir = ROOT / "projects" / "smoke" / "en_ru" / "input"
        self.assertTrue((input_dir / "corpus_en_ru.csv").exists())
        for lang in ("en", "ru"):
            for pos_name in ("noun", "verb", "adj", "adverb"):
                self.assertTrue(
                    (input_dir / f"wordlist_{lang}_{pos_name}.csv").exists()
